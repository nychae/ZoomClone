...
document.querySelector('#addRoomPopup > .footer > button').addEventListener('click', () => {
    ...
    if(roomName.value === '' || capacity === '' || capacity < 2) {
        alert('값을 제대로 입력해주세요');
        return;
    }
})
...
const drawRoomList = () => {
    api.getRooms().then((res) => {
        ...
        if(res.data.length === 0) {
            document.querySelector('.listWrapper').innerHTML = '<div class="empty">No chat rooms available.</div>';
        }
    })
}

const addRoom = (roomName, total) => {
    const room = document.createElement('div');
    room.innerHTML = `<div class="header">
                            <div class="line"></div>
    ...
    room.dataset.roomName = roomName;

    document.querySelector('#roomList > .listWrapper').appendChild(room);
}
